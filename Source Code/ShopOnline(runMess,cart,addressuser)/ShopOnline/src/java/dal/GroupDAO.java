...
public class GroupDAO {
    ...
    public ArrayList<GroupChat> getGroupChat() {          //lấy tin nhắn cuối cùng được gửi trong 1 group chat để hiển thị
        ...
        try {
            String sql = "SELECT tb2.* FROM\n"
                    ...
                    + "LEFT join\n"
                    + "(SELECT  mrID,groupName,m.messageBody,m.createDate,groupID,m.messageID,isRead,status FROM\n"
                    + "(SELECT mr.messageID,mr.mrID,`group`.`groupName`,groupID,mr.isRead,status FROM message_recipient mr INNER JOIN `Group` \n"
                    + "ON `Group`.`groupID` = mr.recipientGroupID) as rs1 INNER JOIN messages m\n"
    ...
    public void updateStatusGroup(String groupID, String status) {
        String sql = "update `group` set status = ? WHERE groupID = ?";
